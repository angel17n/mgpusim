...
var registerBankReportFlag = flag.Bool("report-register-bank", false,
	"Report the number accesses to register bank in each compute unit.")
...
type registerBankTracer struct {
	tracer *rBankTracer
	cu     TraceableComponent
}

type registerBankTracerPlus struct {
	tracer *rBankTracerPlus
	cu     TraceableComponent
}
...
type Runner struct {
	...
	registerBankTracers     []registerBankTracer
	registerBankTracersPlus []registerBankTracerPlus
...
func (r *Runner) ParseFlag() *Runner {
	...
	if *registerBankReportFlag {
		r.ReportRegisterBank = true
	}
...
func (r *Runner) defineMetrics() {
	...
	r.addRegisterBankTracer()
	r.addRegisterBankTracerPlus()
...
func (r *Runner) addRegisterBankTracer() {
	if !r.ReportRegisterBank {
		print("No se reporta register bank")
		return
	}

	for _, gpu := range r.platform.GPUs {
		for _, cu := range gpu.CUs {
			tracer := newRBankTracer()
			r.registerBankTracers = append(r.registerBankTracers,
				registerBankTracer{
					tracer: tracer,
					cu:     cu,
				})
...
func (r *Runner) addRegisterBankTracerPlus() {
	if !r.ReportRegisterBank {
		print("No se reporta register bank")
		return
	}
	...
	for _, gpu := range r.platform.GPUs {
		for _, cu := range gpu.CUs {
			tracer := newRBankTracerPlus()
			r.registerBankTracersPlus = append(r.registerBankTracersPlus,
				registerBankTracerPlus{
					tracer: tracer,
					cu:     cu,
				})
			tracing.CollectTrace(cu.(tracing.NamedHookable), tracer)
...
func (r *Runner) reportInstCount() {
	...
	for _, t := range r.instCountTracers {
		...
		r.metricsCollector.Collect(
			t.cu.Name(), "v_mul_count", float64(t.tracer.vmulCount))
	}
}

func (r *Runner) reportRegisterBank() {
	for _, t := range r.registerBankTracers {
		r.metricsCollector.Collect(
			t.cu.Name(), "cu_rb_read_scalar", float64(t.tracer.readScalarCount))

		r.metricsCollector.Collect(
			t.cu.Name(), "cu_rb_write_scalar", float64(t.tracer.writeScalarCount))

		r.metricsCollector.Collect(
			t.cu.Name(), "cu_rb_read_vectorial", float64(t.tracer.readVectorialCount))

		r.metricsCollector.Collect(
			t.cu.Name(), "cu_rb_write_vectorial", float64(t.tracer.writeVectorialCount))

	}

	for _, t := range r.registerBankTracersPlus {

		for i, tr := range t.tracer.readVectorialCount {
			if tr > 0 {
				r.metricsCollector.Collect(
					t.cu.Name(), "cu_rb_read_v"+strconv.Itoa(i), float64(tr))
			}
		}
		for i, tr := range t.tracer.writeVectorialCount {
			if tr > 0 {
				r.metricsCollector.Collect(
					t.cu.Name(), "cu_rb_write_v"+strconv.Itoa(i), float64(tr))
			}
		}
	}
}

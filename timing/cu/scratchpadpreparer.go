...
func (p *ScratchpadPreparerImpl) prepareSOP1(
	...
	p.readOperand(inst.Src0, wf, 0, scratchPad[0:8], "SRC0")
	layout.SCC = wf.SCC
	layout.EXEC = wf.EXEC
}
...
func (p *ScratchpadPreparerImpl) prepareSOP2(
	...
	p.readOperand(inst.Src0, wf, 0, scratchPad[0:8], "SRC0")
	p.readOperand(inst.Src1, wf, 0, scratchPad[8:16], "SRC1")
...
func (p *ScratchpadPreparerImpl) prepareVOP1(
	...
	offset := 528
	for i := 0; i < 64; i++ {
		p.readOperand(inst.Src0, wf, i, sp[offset:offset+8], "SRC0")
		offset += 8
	}
}
...
func (p *ScratchpadPreparerImpl) prepareVOP2(
	...
	layout.VCC = wf.VCC
	if inst.Src2 != nil {
		p.readOperand(inst.Src2, wf, 0, sp[1552:1560], "SRC2")
	}
	dstOffset := 8
	src0Offset := 528
	src1Offset := 1040
	for i := 0; i < 64; i++ {
		p.readOperand(inst.Dst, wf, i, sp[dstOffset:dstOffset+8], "DEST")
		dstOffset += 8
		p.readOperand(inst.Src0, wf, i, sp[src0Offset:src0Offset+8], "SRC0")
		src0Offset += 8
		p.readOperand(inst.Src1, wf, i, sp[src1Offset:src1Offset+8], "SRC1")
		src1Offset += 8
	}
}
...
func (p *ScratchpadPreparerImpl) prepareVOP3a(
	...
	for i := 0; i < 64; i++ {
		...
		src1Offset += 8
		if inst.Src2 != nil {
			p.readOperand(inst.Src2, wf, i, sp[src2Offset:src2Offset+8], "SRC2")
			src2Offset += 8
		}
	}
}
...
func (p *ScratchpadPreparerImpl) prepareFlat(
	...
	for i := 0; i < 64; i++ {
		p.readOperand(inst.Addr, wf, i, sp[8+i*8:8+i*8+8], "ADDR")
		p.readOperand(inst.Data, wf, i, sp[520+i*16:520+i*16+16], "DATA")
	}
}
...
func (p *ScratchpadPreparerImpl) prepareSMEM(
	...
	if inst.Opcode >= 16 && inst.Opcode <= 26 { // Store instructions
		p.readOperand(inst.Data, wf, 0, scratchpad[0:16], "DATA")
	}

	p.readOperand(inst.Offset, wf, 0, scratchpad[16:24], "OFFS")
	p.readOperand(inst.Base, wf, 0, scratchpad[24:32], "BASE")
}
...
func (p *ScratchpadPreparerImpl) prepareSOPP(
	...
	layout.EXEC = wf.EXEC
	layout.VCC = wf.VCC
	p.readOperand(inst.SImm16, wf, 0, scratchPad[16:24], "SIMM")
}
...
func (p *ScratchpadPreparerImpl) prepareSOPK(
	...
	layout.SCC = wf.SCC
	p.readOperand(inst.Dst, wf, 0, scratchPad[0:8], "DEST")
	p.readOperand(inst.SImm16, wf, 0, scratchPad[8:16], "SIMM")
}
...
func (p *ScratchpadPreparerImpl) prepareDS(
	...
	offset := 8
	for i := 0; i < 64; i++ {
		p.readOperand(inst.Addr, wf, i, sp[offset+i*4:offset+i*4+4], "ADDR")
	}

	if inst.Data != nil {
		offset = 8 + 64*4
		for i := 0; i < 64; i++ {
			p.readOperand(inst.Data, wf, i, sp[offset+i*16:offset+i*16+16], "DATA")
		}
	}
	...
	if inst.Data1 != nil {
		offset = 8 + 64*4 + 256*4
		for i := 0; i < 64; i++ {
			p.readOperand(inst.Data1, wf, i, sp[offset+i*16:offset+i*16+16], "DATA1")
		}
	}
}
...
func (p *ScratchpadPreparerImpl) commitSOP1(
	...
	p.writeOperand(inst.Dst, wf, 0, scratchpad[8:16], "DEST")
	wf.EXEC = layout.EXEC
	wf.SCC = layout.SCC
}
...
func (p *ScratchpadPreparerImpl) commitSOP2(
	...
	scratchpad := instEmuState.Scratchpad()
	layout := scratchpad.AsSOP2()
	p.writeOperand(inst.Dst, wf, 0, scratchpad[16:24], "DEST")
	wf.SCC = layout.SCC
}
...
func (p *ScratchpadPreparerImpl) commitVOP1(
	...
	for i := 63; i >= 0; i-- {
		...
		offset := 8 + i*8
		p.writeOperand(inst.Dst, wf, i, scratchpad[offset:offset+8], "DEST")
	}
}
...
func (p *ScratchpadPreparerImpl) commitVOP3a(
	...
	for i := 63; i >= 0; i-- {
		...
		offset := 8 + i*8
		p.writeOperand(inst.Dst, wf, i, sp[offset:offset+8], "DEST")
	}
}
...
func (p *ScratchpadPreparerImpl) commitVOP3aCmp(
	...
	p.writeOperand(inst.Dst, wf, 0, sp[8:16], "DEST")
}
...
func (p *ScratchpadPreparerImpl) commitVOP3b(
	...
	p.writeOperand(inst.SDst, wf, 0, insts.Uint64ToBytes(layout.SDST), "SDEST")
}
...
func (p *ScratchpadPreparerImpl) commitFlat(
	...
	if inst.Opcode < 24 || inst.Opcode > 31 { // Skip store instructions
		for i := 0; i < 64; i++ {
			...
			p.writeOperand(inst.Dst, wf, i, scratchpad[1544+i*16:1544+i*16+16], "DEST")
		}
	}
}
...
func (p *ScratchpadPreparerImpl) commitSMEM(
	...
	if inst.Opcode <= 12 { // Load instructions
		p.writeOperand(inst.Data, wf, 0, scratchpad[32:96], "DEST")
	}
}
...
func (p *ScratchpadPreparerImpl) commitSOPK(
	...
	inst := instEmuState.Inst()
	scratchpad := instEmuState.Scratchpad()
	p.writeOperand(inst.Dst, wf, 0, scratchpad[0:8], "DEST")
...
func (p *ScratchpadPreparerImpl) commitDS(
	...
	if inst.Dst != nil {
		...
		for i := 0; i < 64; i++ {
			...
			p.writeOperand(inst.Dst, wf, i, sp[offset+i*16:offset+i*16+16], "DEST")
		}
	}
}
...
func (p *ScratchpadPreparerImpl) readOperand(
	...
	case insts.RegOperand:
		p.readReg(operand.Register, operand.RegCount, wf, laneID, buf, regType)
...
func (p *ScratchpadPreparerImpl) writeOperand(
	...
	p.writeReg(operand.Register, operand.RegCount, wf, laneID, buf, regType)
}
